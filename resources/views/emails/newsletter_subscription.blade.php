...
<head>
    ...
    <title>Welcome to VestaNest Newsletter</title>
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .unsubscribe { text-align:center; margin-top:30px; padding:20px; background:#fef3c7; border-radius:10px; }
        .unsubscribe a { color:#f97316; text-decoration:none; font-weight:500; }
    ...
    <div class="email-container">
        <div class="header">
            <div class="logo">
                ...
                <div class="type-badge">Newsletter Subscription</div>
            </div>
        </div>
        <div class="content">
            <div class="greeting">Welcome {{ $subscription['first_name'] ?? 'there' }}! 🎉</div>
            <div class="message">
                Thank you for subscribing to the VestaNest newsletter! You're now part of our community and will receive the latest property updates, market insights, and exclusive offers.
            </div>
            <div class="panel">
                <h3>📬 Subscription Details</h3>
                <ul class="list">
                    <li><strong>Email:</strong> {{ $subscription['email'] ?? '-' }}</li>
                    <li><strong>Name:</strong> {{ ($subscription['first_name'] ?? '') . ' ' . ($subscription['last_name'] ?? '') }}</li>
                    <li><strong>Subscribed:</strong> {{ $subscription['subscribed_at'] ?? 'Just now' }}</li>
                    <li><strong>Status:</strong> Active</li>
                </ul>
                @if(!empty($subscription['preferences']))
                <div style="margin-top:15px;">
                    <strong>Your Preferences:</strong><br>
                    @if(isset($subscription['preferences']['frequency']))
                        <span style="color:#f97316;">Frequency: {{ ucfirst($subscription['preferences']['frequency']) }}</span><br>
                    @endif
                    @if(isset($subscription['preferences']['property_types']))
                        <span style="color:#f97316;">Property Types: {{ implode(', ', $subscription['preferences']['property_types']) }}</span><br>
                    @endif
                    @if(isset($subscription['preferences']['locations']))
                        <span style="color:#f97316;">Locations: {{ implode(', ', $subscription['preferences']['locations']) }}</span>
                    @endif
                </div>
                @endif
            </div>
            <div class="support">
                ...
                <p>You'll receive curated property listings, market trends, investment tips, and exclusive early access to new properties that match your preferences.</p>
            </div>
            <div class="unsubscribe">
                <p style="color:#7c2d12; margin-bottom:10px;">Changed your mind?</p>
                <a href="{{ config('app.url') }}/unsubscribe?email={{ $subscription['email'] ?? '' }}">Unsubscribe from our newsletter</a>
            </div>
